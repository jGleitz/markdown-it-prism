...
describe('code highlighting', () => {
	...
	Object.entries(codeSectionTypeSettings).forEach(([codeSectionType, options]) =>
		describe(`${codeSectionType} code`, () => {
			...
			it('preserves suffixes after the language specification', async () => {
				expect(markdownit()
					.use(markdownItPrism, options)
					.render(await read(`input/${codeSectionType}/with-suffix.md`)),
				).toEqual(await (read(`expected/${codeSectionType}/with-suffix.html`)))
			})
		}))
